...
- 위험/주문 로직 변경 시 **거절 케이스 테스트 필수**. 현재 통과 테스트(18개)는 회귀 가드:
  상태기계, 기본흐름(PENDING→approve→FILLED), 자동주문, KillSwitch 거절, 거래정지/유동성 거절,
  일일손실 한도 차단, 타임아웃 재동기화, 부분 체결 수렴, T+2 결제, 포지션 갱신, 전략 재평가, 스모크.
...
2. 토스증권 실제 API 연동(인증/주문/취소/잔고/체결 조회)으로 `MockTossBrokerAdapter` 대체 — **공식 문서 확인 후**.
